...
const Categorylist = () => {
  ...
  const addEditCategory = (values) => {
    ...
    if (initialVal.id > 0) {
      ...
      const resp = masterService.editCategory(reqeustParam);
      resp
        .then((res) => {
          if (res.status === 200) {
            listData();
      ...
      const resp = masterService.addCategory(reqeustParam);
      resp
        .then((res) => {
          if (res.status === 200) {
  ...
  const onOKConfirm = () => {
    const reqeustParam = { category_id: initialId };
    const resp = masterService.deleteCategory(reqeustParam);
    resp
      .then((res) => {
        if (res.status === 200) {
          setModalVisibleConfirmation(false);
